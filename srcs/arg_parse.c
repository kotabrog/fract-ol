...
/*   Updated: 2021/07/11 16:09:59 by ksuzuki          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
the default values will be applied. (a = 0.3, b = 0.2)\n", 1);
	ft_putendl_fd("- s (square)", 1);
	ft_putendl_fd("example: ./fractol s\n", 1);
	exit(EXIT_SUCCESS);
}
...
void	arg_parse(t_vars *vars, int argc, char **argv)
{
	...
	if ((argv[1][0] == 'm' || argv[1][0] == 's') && argc > 2)
		print_help_and_exit();
	else
	{
		...
		vars->param[1] = 0.1;
		vars->param[2] = 2;
		if (argc > 2 && set_number(argv[2], &(vars->param[0])))
			print_help_and_exit();
